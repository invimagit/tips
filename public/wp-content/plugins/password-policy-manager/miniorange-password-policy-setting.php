...
class MOPPM
{
    ...
    function moppm_add_plugin_action_links($links, $file) {
        if ($file == plugin_basename(dirname(__FILE__) . '/miniorange-password-policy-setting.php')) {
  ...
  function webroom_wc_remove_password_strength() {
    ...
    function moppm_notices(){
       ...
       (function () {
        ...
        jQuery("#moppm-bf-dissmiss-permanent").click(()=>{
                ...
                'nonce'						:  '<?php echo wp_create_nonce('moppm-remove-offer-banner');?>'
                };
    ...
    function moppm_settings_style($hook)
    {
        if (strpos($hook, 'page_moppm')) {
            wp_enqueue_style('moppm_admin_settings_style', plugins_url('includes'.DIRECTORY_SEPARATOR.'css'.DIRECTORY_SEPARATOR.'moppm_style_settings.css', __FILE__),[],MOPPM_VERSION);
            wp_enqueue_style('moppm_admin_settings_datatable_style', plugins_url('includes'.DIRECTORY_SEPARATOR.'css'.DIRECTORY_SEPARATOR.'jquery.dataTables.min.css', __FILE__),[],MOPPM_VERSION);
        }
        wp_enqueue_style('moppm_admin_offers', plugins_url('includes'.DIRECTORY_SEPARATOR.'css'.DIRECTORY_SEPARATOR.'moppm_offers.css', __FILE__),[],MOPPM_VERSION);


    }
    function moppm_settings_script($hook)
    {
        wp_enqueue_script('moppm_admin_settings_script', plugins_url('includes'.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR.'moppm_settings_page.js', __FILE__), array('jquery'),[],MOPPM_VERSION);
        if (strpos($hook, 'page_moppm')) {
            wp_enqueue_script('moppm_admin_datatable_script', plugins_url('includes'.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR.'jquery.dataTables.min.js', __FILE__), array('jquery'),[],MOPPM_VERSION);
        }
    }
